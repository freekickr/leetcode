...
public class SubdomainVisitCount {
    ...
    public static List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (int i = 0; i < cpdomains.length; i++) {
            String[] expression = cpdomains[i].split(" ");
            int value = Integer.parseInt(expression[0]);
            domains.put(expression[1], value);
            while (expression[1].contains(".")) {
                expression[1] = expression[1].substring(expression[1].indexOf(".") + 1);
                if (domains.containsKey(expression[1]))
                    domains.put(expression[1], domains.get(expression[1]) + value);
                else
                    domains.put(expression[1], value);
            }
        }
        for (Map.Entry<String, Integer> pair : domains.entrySet()) {
