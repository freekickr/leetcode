...
public class MaxIncrToKeepSkyline {
    public static int maxIncreaseKeepingSkyline(int[][] grid) {
        int result = 0;
        int onRowsMax[] = new int[grid[0].length];
        int onColumnsMax[] = new int[grid.length];

        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                if (grid[i][j] > onRowsMax[i])
                    onRowsMax[i] = grid[i][j];
                if (grid[i][j] > onColumnsMax[j])
                    onColumnsMax[j] = grid[i][j];
            }
        }

        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                if (grid[i][j] < onRowsMax[i] && grid[i][j] < onColumnsMax[j]) {
                    int add = Math.min(onRowsMax[i], onColumnsMax[j]) - grid[i][j];
                    grid[i][j] += add;
                    result += add;
                }
            }
